...
bool Engine::tableExists(const string &name) {
        filesystem::path path =
            filesystem::path("db") / "tables" / (name + ".meta");
...
bool Engine::createColumnFiles(const string &name,
        ...
        for (auto &col : columns) {
                ...
                if (!file.is_open()) {
                        cout << "Error creating column file:" << col.name
                             << ".col" << endl;
                        return false;
                }
                file.close();
...
bool Engine::createTable(const string &name, const vector<column> &columns,
        ...
        if (!writeMeta(name, columns)) {
                cout << "Failed to write metadata for " << name << ".meta file"
                     << endl;
                return false;
        }
